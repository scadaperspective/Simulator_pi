...
void Dlg::OnStop(wxCommandEvent& event) {
	...
	m_SliderRudder->SetValue(0); // Rudder Centre Position 
	m_textCtrlRudderStbd->SetValue(_T(""));
	m_textCtrlRudderPort->SetValue(_T(""));
...
// AIS ROT Display
	...
	myDirMag = myDir - 11; // Compass Mag Var into here
...
//	    PushNMEABuffer(MWD + _T("\n"));
	...
	RMC = createRMCSentence(mdt, initLat, initLon, initSpd, myDir, myDirMag);
...
//wxString Dlg::createGSVSentence(double satNum1, double satNum2, double satNum3, double satNum4, double satInView ){
	...
	wxString Dlg::createGSVSentence(){
             /**
...
//        nForCheckSum = nGSV + nC + nsF+ nC + nsL + nC + satInView + nC +  pRn1 + nC + eL1 + nC + aZ1 + nC + snR1 + nC + pRn2 + nC + eL2 + nC + aZ2 + nC+ snR2+ nC + pRn3 + nC + eL3 + nC + aZ3 + nC + snR3 + nC + pRn4 + nC + eL4 + nC + aZ4 + nC + snR4;
	...
	wxString Dlg::createHDMSentence(double myDirMag){
		...
		*/
		...
		nForCheckSum = nHDT + nDirMag + nC + nM;
	...
	wxString Dlg::createXDRAWSentence(double air, double water){
		...
		*/
		...
		nForCheckSum = nRSA + nA + nAir + nType + nIDAir + nC + nA + nWater + nType + nIDWater;
		nFinal = ndlr + nForCheckSum + nast + makeCheckSum(nForCheckSum);
		return nFinal;
...
wxString Dlg::createVHWSentence(double stw, double hdg, double hdm, double vKmhr){
/*
...
vKmhr = stw * 1.852; // Speed Through Water in km/hr
	...
	nDirMag = wxString::Format(_T("%3.1f"), hdm); // Degrees Magnetic Heading
...
wxString Dlg::createMWVASentence(double spd, double hdg, double winddirection, double windspeed){
	...
	*/
	...
	nSpd = wxString::Format(_T("%3.1f"), aws); // Applied Wind Speed Value
	nDir = wxString::Format(_T("%3.1f"), awa); //Applied Wind Angle Value

	nForCheckSum = nMWV + nDir + nC + nRelTrue + nSpd + nC + nUnits + nValid;
	nFinal = ndlr + nForCheckSum + nast + makeCheckSum(nForCheckSum);
	return nFinal;
...
wxString Dlg::createMWVTSentence(double spd, double hdg, double winddirection, double windspeed){
	...
	*/
	...
	nForCheckSum = nMWD + nDir + nC + nTrue + nSpd + nC + nUnits + nValid;
	nFinal = ndlr + nForCheckSum + nast + makeCheckSum(nForCheckSum);
	return nFinal;
...
wxString Dlg::createRMCSentence(wxDateTime myDateTime, double myLat, double myLon, double mySpd, double myDir, double myDirMag){
...
*/
	...
	nForCheckSum = nRMC + nTime + nC + nA + nC + nNS + nEW + nSpd + nC + nDir + nC + nDate +  nC + nDirMag + nC + nA ;
	nFinal = ndlr + nForCheckSum + nast + makeCheckSum(nForCheckSum);
	return nFinal;
}
